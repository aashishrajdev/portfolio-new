...
export class SpriteTracker {
  ...
  async _load(src) {
    ...
    // Prefer fetch -> blob -> createImageBitmap: the blob decode is specified
    // to run asynchronously (off the main thread in practice), and slicing
    // cells from a decoded ImageBitmap is a cheap crop. Slicing from an <img>
    // element instead can re-rasterise the full ~13-megapixel sheet for every
    // cell, which is exactly the kind of main-thread stall that fights the
    ...
    try {
      ...
      for (let i = 0; i < frames; i += BATCH) {
        const slice = await Promise.all(
          Array.from({ length: Math.min(BATCH, frames - i) }, (_, k) => {
            const n = i + k;
            return createImageBitmap(source, (n % cols) * fw, Math.floor(n / cols) * fh, fw, fh);
          }),
        );
        cells.push(...slice);
        if (this.destroyed) {
        ...
        // Yield between batches — during load the page has better things to do.
        await new Promise((r) =>
      ...
      return { sheet: source, cells: null };
    }
  }
